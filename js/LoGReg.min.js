  ...
  function register(){var e=document.getElementById("register-email").value,r=document.getElementById("register-password").value,i=document.getElementById("register-displayname").value;if(!e||!r||!i){alert("Please fill in all the required fields.");return}firebase.auth().createUserWithEmailAndPassword(e,r).then(e=>{var r=e.user;r.updateProfile({displayName:i}).then(()=>{r.sendEmailVerification().then(()=>{alert("Registration successful. Verification email sent."),window.location.href="https://www.codemind.eu.org/p/login_5.html"}).catch(e=>{alert("Error sending verification email: "+e.message)})}).catch(e=>{alert("Error updating display name: "+e.message)})}).catch(e=>{alert("Error registering user: "+e.message)})}
